# Original code written by by Johan G. McQuillan (2018)
...
class PayrollCommitment(object):
    ...
    ALLOWANCE = 12509  # Personal allowance (reduces taxable income)
...
class TaxCode(Tax):
    ...
    def take_home_pay(self, income):
        return income - self.tax(income) - self.pension_cost(income)

# Brackets and rates taken from:
# https://www.gov.uk/guidance/rates-and-thresholds-for-employers-2019-to-2020
# Last updated 9th of May 2019 (2019-2020 tax year)
...
StudentFinance = TaxBracket('Student Finance',
                            [25725],  # Based on Plan 2 undergraduate loans
                            [0.09])
...
def generate_tax_code(student=False, pension=True, statutory_pension=0.03, private_pension=None):
    ...
    :rtype: TaxCode
    """
    if statutory_pension is None:
        statutory_pension = 0.03
    tax_code = TaxCode()
